...
#pragma omp parallel num_threads(NUM_THREADS) default(none) shared(input_img, output_img) \
firstprivate(kernel, kernel_vec, kernel_vec8, num_cols, num_rows)
        {
                ...
                for (std::int32_t j = 15; j < num_cols - 1; j += VEC_SIZE) {
                    ...
                    for (std::int32_t di = 0; di <= 1; di++) {
                        for (std::int32_t dj = -1; dj <= 1; dj++) {
                            ...
                            sum = _mm512_fmadd_ps(kernel_vec[di][dj + 1], img_val, sum);
                        }
                    }
...
#pragma omp single nowait
            {
                for (std::int32_t i = 1; i < num_rows - 1; i++) { // 6 multiply and adds
                    output_img[i * num_cols + num_cols - 1] = kernel[0][0]*input_img[(i-1)*num_cols + num_cols - 2] +
                ...
                for (int j = 1; j <= 6; ++j) {
                    for (int di = -1; di <= 1; ++di) {
                        for (int dj = -1; dj <= 1; ++dj) {
                            output_img[i * num_cols + j] += kernel[di + 1][dj + 1] * input_img[(i + di) * num_cols + j + dj];
                        }
                    }
                }
